...
def clone_and_upload_hf_repo(config):
    ...
    if not hf_token:
        raise ValueError(
            "Hugging Face token is missing. Please check your config file.")
    ...
    for hf_repo_url in config['repositories']:
        ...
        try:
            ...
            new_repo_id = f"{organization}/{repo_name}"
            print(
                f"Creating new model repository under organization {organization}...")
            repo_url = api.create_repo(
                repo_id=new_repo_id, exist_ok=True).git_url

            remote_url_with_creds = repo_url.replace(
                "https://", f"https://{hf_username}:{hf_token}@")

            os.chdir(cloned_repo_dir)
            subprocess.run(["git", "init"], check=True)
            subprocess.run(["git", "remote", "add", "origin",
                           remote_url_with_creds], check=True)
            subprocess.run(["git", "add", "."], check=True)
            subprocess.run(
                ["git", "commit", "-m", "populate repo"], check=True)
            subprocess.run(["git", "branch", "-M", "main"], check=True)
            subprocess.run(["git", "push", "-u", "origin", "main"], check=True)

        except (GitCommandError, Exception) as e:
            print(
                f"Error occurred while processing repository {hf_repo_url}: {e}")
        finally:
            if os.path.exists(cloned_repo_dir):
                print(
                    f"Cleaning up {cloned_repo_dir} due to error or completion.")
